...
const getClientFireStoreData = async (setState, collection) => {
  try {
    ...
    // Fetch all the protocols in one go.
    const protocolRefs = clientData.map(doc => doc.data().protocol);
    const protocolDocs = await Promise.all(protocolRefs.map(ref => getDoc(ref)));
